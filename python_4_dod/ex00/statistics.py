...
def ft_statistics(*args: Any, **kwargs: Any) -> None:
    """
    Computes and prints various statistical measures of given numeric
    data based on specified options. The function accepts variable
    positional and keyword arguments for data input and statistical
    options respectively.

    Args:
        *args (Any): A variable number of numeric arguments representing
            the dataset for statistical computations. Non-numeric
            elements will raise an error.
        **kwargs (Any): A variable number of keyword arguments where the
    ...
    Returns:
        ...
        results of the specified statistical computations.

    Raises:
        TypeError: If non-numeric arguments are passed in *args or
            invalid values are specified for **kwargs.
    """
    ...
    def get_percentile(percentile: float) -> float:
        ...
        lower_idx = int(idx)
        upper_idx = (lower_idx + 1 if lower_idx < data_len - 1
                     else lower_idx)
        weight = idx - lower_idx
        return float(
            data[lower_idx] + weight * (data[upper_idx] -
                                        data[lower_idx]))
...
def print_std(data: list[Any], data_len: int) -> None:
    """
    Calculates the standard deviation of a dataset and prints the
    result.
    ...
    Args:
        data (list[Any]): A list of numeric elements for which the
...
def print_quartile(get_percentile: Callable[..., float]) -> None:
    """
    Calculates and prints the first and third quartiles using a
    percentile fetching function passed as an argument.

    This function takes a callable object capable of computing
    percentiles and retrieves the 25th percentile (Q1) and the 75th
    percentile (Q3). It then prints these values formatted as
    "quartile : {q1} {q3}".

    Args:
        get_percentile (Callable[..., float]): A callable function that
            calculates and returns the specified percentile when given
...
def print_median(data: list[Any], data_len: int) -> None:
    ...
    This function computes the median of a sorted dataset. For
    even-length datasets, the median is calculated as the sum of the
    two middle elements. For odd-length datasets, the median is the
    middle element. The computed median is then printed to the console.

    Args:
        data: List of sorted elements from which the median is to be
            calculated.
...
def print_mean(data: list[Any], data_len: int) -> None:
    ...
    This function takes a list of numerical data and its length as
    input, calculates the mean value, and prints it to the console.

    Args:
        data (list[Any]): The dataset containing numerical values for
            which the mean is to be calculated.
        data_len (int): The length of the dataset. It is expected to
            match the length of the data list.
